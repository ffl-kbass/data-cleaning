...
const States = () => {
  ...
  useEffect(() => {
    ...
    temp = items
      .sort(
        (a, b) =>
          a.data.percent_districts_clean - b.data.percent_districts_clean
      )
      .slice(0, 3);
        ...
        <div className="grid h-full grid-rows-2 gap-2">
          <Stats title="Overall Cleanliness">
            <div className="flex w-full flex-row flex-nowrap items-center gap-2">
              <div className="h-2 flex-grow rounded-full bg-slate-300">
                <div
                  style={{
                    width:
                      (data
                        ? Number(
                            data.states.national.percent_districts_clean
                          ).toFixed(2)
                        : 0) + "%",
                  }}
              ...
              <p className="text-base">
                {data &&
                  Number(data.states.national.percent_districts_clean).toFixed(
                    2
                  )}
                %
              </p>
          ...
          <Stats title="Meeting Goal">
            <div className="flex w-full flex-row flex-nowrap items-center gap-2">
                ...
                <div
                  style={{
                    width:
                      (data
                        ? Number(
                            data.states.national.percent_meeting_goal
                          ).toFixed(2)
                        : 0) + "%",
                  }}
                  className="h-full rounded-full bg-green-500"
              ...
              <p className="text-base">
                {data &&
                  Number(data.states.national.percent_meeting_goal).toFixed(2)}
                %
              </p>
            </div>
          </Stats>
